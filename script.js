// --- ！！！ここにあなたのPexelsアクセスキーを設定してください！！！ ---
const PEXELS_API_KEY = "KNagZbdEkUOFTDUFzAVPvxu6p8rMQR2MKUxR3tMhatctElZavbSnA0H2";

// --- AI / 写真背景生成 ---
async function generateAndSetBackground() {
    if (!PEXELS_API_KEY || PEXELS_API_KEY === "YOUR_PEXELS_API_KEY") {
        alert("Pexels APIキーが設定されていません。script.jsの先頭を編集してください。");
        return;
    }

    const selectedEmojis = Array.from(activeSounds.keys());
    const query = generateImageSearchQuery(selectedEmojis); // 既存の関数を利用
    if (!query) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Pexels API エラー: ${response.status}`);
